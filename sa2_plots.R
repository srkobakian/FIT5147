...
aus_liver_f <- SIR_females %>% 
  ...
  ggplot() + 
  geom_sf(aes(fill = colour_SIR), colour = "white") + theme_map() + theme(plot.background = element_blank())
...
thyroid_colours <- function(sir_p50){
  value <- case_when(
    sir_p50 <  0.74 ~ "#33809d",
...
aus_thyroid_f <- SIR_females %>% 
  mutate(colour_SIR = map(Thyroid, thyroid_colours)) %>% 
  ggplot() + 
  geom_sf(aes(fill = colour_SIR), colour = "white") + theme_map() + theme(plot.background = element_blank())

ggsave(filename = "figures/aus_thyroid_f.png", plot = aus_thyroid_f,
       device = "png", bg = "transparent", dpi = 300,  width = 10, height = 10)
